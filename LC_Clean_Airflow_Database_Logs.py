# Adapted and Tested on Airflow Version 1.10.2
from airflow.models import DAG, DagRun, TaskInstance, Log, XCom, SlaMiss,TaskReschedule
...
airflow trigger_dag --conf '{"maxDBEntryAgeInDays":5}' airflow-db-cleanup
--conf options:
    maxDBEntryAgeInDays:<INT> - Optional
"""

DAG_ID = os.path.basename(__file__).replace(".pyc", "").replace(".py", "")  # airflow-db-cleanup
START_DATE = timezone.utcnow() - timedelta(minutes=1)
...
ALERT_EMAIL_ADDRESSES = ["admin@deliverbi.co.uk"]              # List of email address to send email alerts to if this job fails
DEFAULT_MAX_DB_ENTRY_AGE_IN_DAYS = 5   # Length to retain the log files if not already provided in the conf. If this is set to 30, the job will remove those files that are 30 days old or older.
...
default_args = {
    ...
    'start_date': datetime(2019, 1, 30, 12, 10),
    'retries': 1,
    'retry_delay': timedelta(minutes=1)
}

dag = DAG(DAG_ID, default_args=default_args, schedule_interval='30 2 * * *',catchup=False)
